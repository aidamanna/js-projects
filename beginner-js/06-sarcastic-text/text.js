...
function transformText(text) {
    ...
    const transformedText = Array.from(text).map(filters[filter]).join("");
    result.textContent = transformedText;
}

textarea.addEventListener('input', event => transformText(event.target.value));

filterInputs.forEach(filterInput => filterInput.addEventListener('input', () => { transformText(textarea.value)}));